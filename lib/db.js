...
export async function getSubmissionsPage({
  ...
  const conds = [];
  if (q && String(q).trim()) {
    const raw = String(q).trim().replace(/[%_]/g, (c) => "\\" + c);
    const term = `%${raw}%`;
    conds.push(db`(name ILIKE ${term} ESCAPE '\\' OR phone ILIKE ${term} ESCAPE '\\' OR zone_no ILIKE ${term} ESCAPE '\\' OR zone_name ILIKE ${term} ESCAPE '\\')`);
  }
